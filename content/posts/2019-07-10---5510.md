...
![image](/media/POST/5510/2.jpg)

Surface Potential (표면 퍼텐셜)q

$$
e\phi_s = E_{Fi}(x\rightarrow\infty)-E_{Fi}(x=0)
$$

Gate 전압이 인가되면, oxide-semiconductor 경계면(표면)에서 발생 Gate 전압 상승 시 $\phi_s$ 상승한고,  Depletion 두께는 다음과 같다.
...
[MOS Capacitor](https://www.edison.re.kr/scienceappstore/-/scienceapp/tbm/1-0-0/view) SW을 이용하여 MOS 구조의 gate 전압을 각각 -2V~2V까지 0.2V 간격으로 변화하면서 gate 전압에 대한 표면 potential 변화를 살펴보자. (단, $N_a = 10^{16} \mathrm{cm}^{-3}$)
...
Flat band 전압($V_{FB}$)은 semiconductor의 band bending이 없도록 하는 gate 전압으로 Gate 전압이 0이라도 band bending 발생한다. Metal-semiconductor 간의 work function 차이와 Oxide 내부의 trapped charge가 발생 주요 원인이다.

$$
V_{FB}=\phi_{ms}-\dfrac{Q^{'}_{ss}}{C_{ox}}
$$

- $Q^{'}_{ss}$은 Oxide 내 단위 면적당 trapped charge
- $C_{ox}$은 Oxide capacitance
